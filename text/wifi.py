...
class breakWiFi(object):
	...
	def writePassword(self, filepath):
		...
		pwds = it.product(astring, repeat=5)  # 8位密码长度
		with open(filepath, 'w', encoding='utf-8') as f:
			for pwd in pwds:
				f.write('055886' + ''.join(pwd))
				f.write(''.join('\n'))
	...
	def wifiScan(self):
		...
		for w in wifiList:
			wifiNameAndSignal = (
				100 + w.signal, w.ssid.encode('raw_unicode_escape').decode('utf-8'))  # 解决乱码问题并返回元组
			wifiNewList.append(wifiNameAndSignal)

		wifi_signal_and_name_list = sorted(
			wifiNewList, key=lambda i: i[0], reverse=True)  # 按信号强度倒序
		index = 0
